...
async function resolveByIlanzouAccount(resource) {
  ...
  if (!loginRes || loginRes.code !== 200) {
    throw new Error('ilanzou 登录失败: ' + (loginRes?.msg || ''));
  }

  if (!resource.file_id) throw new Error('file_id 为空，无法通过账号模式解析');
  const direct = await client.downloadFile(String(resource.file_id), true);
  if (direct && typeof direct === 'string') return direct.trim();
  if (direct && direct.url) return String(direct.url).trim();
  throw new Error('downloadFile 未返回直链');
}
...
async function resolveByPublicShareUrl(shareUrl, password) {
  ...
  if (password && /pwd|pass|访问密码|请输入密码/.test(pageHtml) && /<form/i.test(pageHtml)) {
    const m = pageHtml.match(/url\s*:\s*['"]([^'"]+)['"]/i) || pageHtml.match(/var\s+url\s*=\s*['"]([^'"]+)['"]/i);
    const skDataMatch = pageHtml.match(/data\s*:\s*({[^}]+pwd[^}]+})/);
  ...
  if (!iframeSrc) throw new Error('未找到 iframe，页面结构可能已更新');
  ...
  const signMatch = iframeHtml.match(/['"]sign['"]\s*:\s*['"]([^'"]+)['"]/)
    || iframeHtml.match(/var\s+(wsk_sign|sasign|ws?i?gn?)\s*=\s*['"]([^'"]+)['"]/);
  const sign = signMatch ? (signMatch[2] || signMatch[1]) : '';
...
async function resolve(resource) {
  ...
  if (resource.source_provider === 'ilanzou' && resource.source_login_type === 'account') {
    try {
      url = await resolveByIlanzouAccount(resource);
    } catch (err) {
      if (isLanzouPublicUrl(resource.share_url)) {
        url = await resolveByPublicShareUrl(resource.share_url, resource.share_pwd);
      } else {
        throw err;
      }
    }
  } else if (isLanzouPublicUrl(resource.share_url)) {
    url = await resolveByPublicShareUrl(resource.share_url, resource.share_pwd);
  } else {
    throw new Error('不支持的来源或 share_url 为空');
  }

  if (!url) throw new Error('直链解析失败');
