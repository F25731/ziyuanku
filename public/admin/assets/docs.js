...
data: {"code":200,"message":"ok","count":20,"next_cursor":"...","has_more":true}`
    },
    {
      ...
      desc: '启用 Manticore 时走轻量搜索索引（只存 id/file_name/source_id/file_type），否则回落 MySQL FULLTEXT。返回字段不含直链，下游拿到 id 后再按需调 link 接口换直链。本接口不消耗 Key 的日/总配额。',
      params: [
...
echo '直链: ' . $link['url'] . PHP_EOL;`
  ...
  notes: [
    ...
    '搜索优先使用 Manticore 轻量索引；外部搜索未启用或短暂不可用时可回落 MySQL FULLTEXT。默认禁止单字搜索，建议下游做 200-300ms 输入防抖。',
  ...
  cleanupDsl: {
    intro: '数据清理用 JSON DSL 配置规则，三个阶段独立可组合：大小过滤 → 格式过滤 → 去重。规则保存在后台 cleanup_rules 表，可导出 JSON 分享。运行流程在后台→数据清理 tab：生成候选集 → 审核样例和统计 → 应用候选（软删除，可撤销）。',
    structure: `{
...
# 单位支持：B / KB / MB / GB / TB（不带单位时按 B；纯数字"1024"=1024 字节）`,
    workflow: [
      ...
      '3. 选规则 + 选范围 + 决定是否跨库 → 点「生成候选」',
      '4. 看顶部进度卡里的 50 条样例和统计，确认"候选 / 同组赢家 / 分组键·分数"对照没误伤',
      '5. 点「应用候选」真删（软删 is_deleted=1，并同步搜索 outbox）',
      '6. 看错了点「撤销」全部恢复'
    ],
    runOrder: 'size_filter → format_filter → 去重。系统先写 cleanup_candidates 候选表，不直接删除；去重阶段先写分组 key，再由 MySQL 选出每组输家。',
    ...
    safety: '单次应用候选最多删除占活跃资源的比例由"安全阈值"控制（默认 30%，可在数据清理 tab 顶部调）。超过阈值时不会删除任何资源，只会提示你确认后再应用。'
  }
};
